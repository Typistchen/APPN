...
def get_stop_epoch(algorithm, dataset, n_shot=5):
    if algorithm in ['baseline', 'baseline++']:
        if dataset in ['omniglot', 'cross_char']:
            ...
            stop_epoch = 5  # This is different as stated in the open-review paper. However, using 400 epoch in baseline actually lead to over-fitting
        else:
            stop_epoch = 5  # default
    else:
        if n_shot == 5:
            stop_epoch = 5
        else:
