...
class TrainDataset(Dataset):
    def __init__(self, subset, dataset):
        ...
        self.df = pd.DataFrame(self.index_subset(self.subset, self.dataset))
        ...
        self.datasetid_to_subset = self.df.to_dict()['class_name']

        self.transform_init = TransformLoader(84)
        if subset == "val":
            self.transform = self.transform_init.get_composed_transform(aug=False)
        elif subset == "train":
            self.transform = self.transform_init.get_composed_transform(aug=True)
        # # Setup transforms
    ...
    def __getitem__(self, item):
        ...
        class_name = self.datasetid_to_subset[item]
        return instance, label , class_name
    ...
    @staticmethod
    def index_subset(subset,dataset):
