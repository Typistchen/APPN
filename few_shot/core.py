...
import numpy as np
import torch
from few_shot.proto import add_noise
...
class EvaluateFewShot(Callback):
    ...
    def __init__(self,
        ...
        self.prefix = prefix
        self.kwargs = kwargs
        self.metric_name = f'{self.prefix}{self.k_shot}-shot_{self.n_way}-way_acc'
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for batch_index, batch in enumerate(self.taskloader):
            x, y, class_name = self.prepare_batch(batch)
            if self.noise_type!= None:
                x = add_noise(self.k_shot, self.n_way, x, self.noise_type, class_name, self.total_class, self.dataset, self.noise_dataset)
...
def prepare_nshot_task(n: int, k: int, q: int) -> Callable:
    ...
    def prepare_nshot_task_(batch: Tuple[torch.Tensor, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        query_y, support_y = create_nshot_task_label(k, q, n)
        query_y = query_y.cuda()

        support_y = support_y.cuda()
        return x, query_y, support_y, class_name

    return prepare_nshot_task_


def create_nshot_task_label(k: int, q: int, n) -> torch.Tensor:
    ...
    """
    query_y = torch.arange(0, k, 1 / q).long()
    support_y = torch.arange(0, k, 1 / n).long()
    return query_y, support_y
