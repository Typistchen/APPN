...
def calculate_distance_sum(arr, index):
    ...
    for i in range(0, len(arr)):
        ...
        tensor1 = F.normalize(arr[i], dim=0)
        tensor2 = F.normalize(arr[index], dim=0)
        # dist = 1 - F.cosine_similarity(tensor1, tensor2, dim=0)
...
def calculate_weighted_proto(scale, embedings, n_way, k_shot):
    emb_proto = torch.zeros(n_way, embedings.shape[-1]).cuda()
    scale = scale.reshape(n_way,-1)
    
    
    for way in range(n_way) :
        for shot in range(k_shot):
            matrix1 = scale[way,shot]
            matrix2 = embedings[way,shot]
            res = matrix1 * matrix2
            emb_proto[way] += res
...
def Remove_incorrect_label(index, score):
    result = []
    for i in index:
        if score[i] > 1.1:
            result.append(i)
...
class Soft_pseudo_labels(nn.Module):
    ...
    def forward(self, inputs):
        ...
        return new_embs, soft_pseudo_labels, confifence
...
class   Graph(nn.Module):
    ...
    def forward(self, inputs):  
        embedings, soft_pseudo_labels, confifence = self.soft_pseudo_labels(inputs)
        ...
        label_all = torch.cat((soft_pseudo_labels, confifence), dim=1)
        ...
        correct_indices = Remove_incorrect_label(index=correct_indices, score=LOF_score).long().cuda()
        ...
        return normalize_confidence, label_all_clean
