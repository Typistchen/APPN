...
def fit(model: Module, optimiser: Optimizer, loss_fn: Callable, epochs: int, dataloader: DataLoader,
    ...
    # 实例化一个CallbackList对象，[DefaultCallback(), ]之所以要加入一个,是因为一个元素无法构成列表，
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_index, batch in enumerate(dataloader):
            ...
            x, y, class_name = prepare_batch(batch)

            loss, y_pred = fit_function(model, optimiser, loss_fn, x, y, **fit_function_kwargs, class_name = class_name)
