...
def add_noise(k_shot, n_way, data, noise_type, class_name, total_class, dataset, noisedataset, model, scale):
    ...
    if noise_type == 'IT':
        ...
        if scale == 1:
            ...
            noise_idxes2 = []
            for i in range(n_way):
                randam1 = random.randint(0, 4)
                random2= random.choice([x for x in range(0, 4) if x != randam1])
                noise_idxes1.append(k_shot * i  + randam1)
                noise_idxes2.append(k_shot * i  + random2)

            temp1 = data[noise_idxes1[0]]
            temp2 = data[noise_idxes2[0]]
            for i in range(len(noise_idxes1)-1):
                data[noise_idxes1[i]] = data[noise_idxes1[i+1]]
                data[noise_idxes2[i]] = data[noise_idxes2[i+1]]


            data[noise_idxes1[-1]] = temp1
